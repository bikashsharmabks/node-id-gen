...
function init(redisClient, opt) {
	...
	if (opt) {
		NodeFlakeGen.options.retryTimes = opt.retryTimes ? opt.retryTimes : NodeFlakeGen.options.retryTimes;
		NodeFlakeGen.options.retryDelay = opt.retryDelay ? opt.retryDelay : NodeFlakeGen.options.retryDelay;
	}
}
...
function next(name) {
	return new Promise(function(resolve, reject) {
		...
		runRequest(sequence,
			NodeFlakeGen.options.retryTimes,
			NodeFlakeGen.options.retryDelay,
...
function runRequest(sequence, retryTimes, retryDelay, callback) {
	var cntr = 0;
